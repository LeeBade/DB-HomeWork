...
";
--ans for Q7
with Expend(customerid,companyname,expenditure,buk) as (
    select customerid,companyname,round(sum(unitprice*quantity),2),ntile(4) over(partition by round(sum(unitprice*quantity),2) order by round(sum(unitprice*quantity),2))
    from customer left outer join 'order' on customer.id='order'.customerid
